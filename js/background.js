...
function setTabIcon(tabId, changeInfo, tab)
{
  if (!option.hideTabIcon.get())
  {
    ...
    else if (strStartsWith(url, "chrome://newtab")||url=="about:blank") showTabIconNewPage(tabId);
...
function showTabIcon(tabId, enable, white)
{
  if (enable)
  {
	...
	//chrome.pageAction.setTitle({ tabId: tabId, title: '123' })
	//console.warn('tabId: '+tabId+' enable: '+enable+' white: '+white);
	/*
	loadAllCookies(whiteList, function (result)
    {
...
function showTabIconNewPage(tabId)
{
    ...
    {
	  var count = '32';
	  if (result.black.length == 1){var count = '1';}
	  if (result.black.length == 2){var count = '2';}
	  if (result.black.length == 3){var count = '3';}
	  if (result.black.length == 4){var count = '4';}
